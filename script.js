...
// ===== FORMULÁRIO LEADS ALUNOS =====

const leadsAlunosForm = document.getElementById('leadsAlunosForm');

if (leadsAlunosForm) {
    leadsAlunosForm.addEventListener('submit', function(e) {
        e.preventDefault();
        
        console.log('📝 Formulário de Leads Alunos submetido!');
        
        // Validação das checkboxes (máximo 3)
        const checkboxes = leadsAlunosForm.querySelectorAll('input[name="funcionalidades"]:checked');
        
        if (checkboxes.length === 0) {
            alert('Por favor, selecione pelo menos 1 funcionalidade essencial.');
            return;
        }
        
        if (checkboxes.length > 3) {
            alert('Por favor, selecione no máximo 3 funcionalidades essenciais.');
            return;
        }
        
        // Validate required fields
        const requiredFields = ['aluno_nome', 'aluno_email', 'aluno_whatsapp', 'aluno_cidade', 
                               'quando_comecar', 'categoria_cnh', 'experiencia_previa', 
                               'orcamento', 'usaria_app', 'quanto_pagaria', 'modelo_pagamento'];
        let isValid = true;
        
        requiredFields.forEach(field => {
            const input = leadsAlunosForm.querySelector(`[name="${field}"]`);
            if (!input.value.trim()) {
                input.style.borderColor = '#EF4444';
                isValid = false;
            } else {
        ...
        // Validate email
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        const emailInput = leadsAlunosForm.querySelector('[name="aluno_email"]');
        if (!emailRegex.test(emailInput.value)) {
            emailInput.style.borderColor = '#EF4444';
            alert('Por favor, insira um e-mail válido.');
            return;
        ...
        // Show loading state
        const submitBtn = leadsAlunosForm.querySelector('button[type="submit"]');
        const originalBtnText = submitBtn.innerHTML;
        submitBtn.disabled = true;
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Enviando...';
        
        // Coleta funcionalidades selecionadas
        const funcionalidades = Array.from(checkboxes).map(cb => cb.value).join(', ');
        
        // Coleta TODOS os dados
        const formData = {
            aluno_nome: document.getElementById('aluno_nome').value,
            aluno_email: document.getElementById('aluno_email').value,
            aluno_whatsapp: document.getElementById('aluno_whatsapp').value,
            aluno_cidade: document.getElementById('aluno_cidade').value,
            quando_comecar: document.getElementById('quando_comecar').value,
            ...
            usaria_app: document.getElementById('usaria_app').value,
            funcionalidades: funcionalidades,
            quanto_pagaria: document.getElementById('quanto_pagaria').value,
            modelo_pagamento: document.getElementById('modelo_pagamento').value,
            tipo_formulario: 'lead_aluno' // Identificador para o Apps Script
        };
        
        console.log('📝 Dados coletados:', formData);
        
        // Cria iframe oculto
        let iframe = document.getElementById('hidden_iframe_aluno');
        if (!iframe) {
            iframe = document.createElement('iframe');
            iframe.id = 'hidden_iframe_aluno';
            iframe.name = 'hidden_iframe_aluno';
            iframe.style.display = 'none';
            document.body.appendChild(iframe);
        }
        ...
        tempForm.method = 'POST';
        tempForm.action = GOOGLE_SCRIPT_URL; // Usa a mesma URL
        tempForm.target = 'hidden_iframe_aluno';
        ...
        Object.keys(formData).forEach(key => {
            ...
            input.value = formData[key];
            tempForm.appendChild(input);
            console.log(`Adicionado: ${key} = ${formData[key]}`);
        });
        ...
        // Aguarda 3 segundos e mostra sucesso
        setTimeout(() => {
            document.body.removeChild(tempForm);
            console.log('✅ Formulário enviado!');
            
            showAlunoModal();
            leadsAlunosForm.reset();
            ...
            if (typeof gtag !== 'undefined') {
                gtag('event', 'form_submission', {
                    'event_category': 'Leads Alunos',
                    'event_label': 'Lead Aluno Cadastrado'
                });
            }
        }, 3000);
    });
...
// ===== MODAL FUNCTIONS ALUNOS =====
function showAlunoModal() {
    const modal = document.getElementById('alunoSuccessModal');
    modal.classList.add('active');
    document.body.style.overflow = 'hidden';
}
...
// Close modal when clicking outside
document.getElementById('alunoSuccessModal')?.addEventListener('click', function(e) {
    if (e.target === this) {
        closeAlunoModal();
    }
});

// Máscara para WhatsApp dos alunos
const alunoWhatsappInput = document.querySelector('input[name="aluno_whatsapp"]');

if (alunoWhatsappInput) {
    alunoWhatsappInput.addEventListener('input', function(e) {
        let value = e.target.value.replace(/\D/g, '');
...
window.showAlunoModal = showAlunoModal;
